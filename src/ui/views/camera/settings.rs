use crate::{
    ...
    settings::Settings,
    ui::state::{camera::ModifiedSettingsMap, AppState},
};
use eframe::{
    egui::{
...
pub fn show(ctx: &Context, state: &mut AppState) {
    ...
    if let Some(camera) = &state.camera {
        SidePanel::right("camera_settings_panel").show(ctx, |ui| {
            ...
            let mut set_setting = |setting: CameraSettings, section_id: i32| {
...
fn display_setting(ui: &mut Ui, setting: &mut CameraSettings, changed: &mut bool, modified: bool) {
    ...
    group.show(ui, |ui| {
        ...
        ui.add_enabled_ui(!setting.readonly, |ui| match &mut setting.widget {
            StaticWidget::Text(ref mut text) => {
                if ui.text_edit_singleline(text).changed() {
                    *changed = true;
                }
            }
            StaticWidget::Radio { choices, ref mut choice } => {
                ComboBox::from_id_source(&setting.label).selected_text(&*choice).show_ui(
                    ui,
                    |ui| {
                        for choices_choice in choices {
                            // TODO: To not use to_owned, maybe use choice a index here?
                            if ui
                                .selectable_value(
                                    choice,
                                    choices_choice.to_owned(),
                                    &*choices_choice,
                                )
                                .changed()
                            {
            ...
            StaticWidget::Toggle { undefined, ref mut value } => {
                // TODO: Make a custom toggle switch with undefined state
                ComboBox::from_id_source(&setting.label)
                    .selected_text(match (undefined, &value) {
                        (true, _) => "Unknown",
                        (false, true) => "Yes",
                        (false, false) => "No",
                    })
                    .show_ui(ui, |ui| {
                        for option in [true, false] {
                            if ui
                                .selectable_value(value, option, if option { "Yes" } else { "No" })
                                .changed()
                            {
                                *changed = true
                            }
            ...
            StaticWidget::Range { range, ref mut value, step } => {
                if ui.add(Slider::new(value, range.to_owned()).step_by(*step as f64)).changed() {
                    *changed = true;
                }
            }
            StaticWidget::Date { ref mut timestamp } => {
                if ui.add(DragValue::new(timestamp)).changed() {
                    *changed = true;
                }
            }
            StaticWidget::Button => {
